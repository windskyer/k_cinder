...
def ssp_lu_clone_simple(cs, cluster_id):
    ...
    (status, source_udid, job_id) = cluster.lu_create(n("P2Z-SOURCE-DM"), 10)
    print ("lu_create job >%s< for SOURCE completed w/ status: >%s<" %
           (job_id, status))

    # CREATE TARGET
    cluster = cs.cluster.get(cluster_id)
    n = cs.sharedstoragepool.create_unique_name
    (status, target_udid, job_id) = cluster.lu_create(n("P2Z-TARGET-DM"), 10)
    print ("lu_create job >%s< for TARGET completed w/ status: >%s<" %
           (job_id, status))
