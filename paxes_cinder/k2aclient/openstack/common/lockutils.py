...
from paxes_cinder.k2aclient.openstack.common import local
...
# :bh:
CONF_lock_path = "/tmp"
CONF_disable_process_locking = False
...
#                                                  'method': f.__name__}) :bh:
                ...
                try:
#                     if external and not CONF.disable_process_locking: :bh:
                    if external and not CONF_disable_process_locking:
                        LOG.debug('Attempting to grab file lock "%(lock)s" '
                                  'for method "%(method)s"...',
                                  {'lock': name, 'method': f.__name__})
#                       LOG.debug(_('Attempting to grab file lock "%(lock)s" '
#                                   'for method "%(method)s"...'),
#                                 {'lock': name, 'method': f.__name__}) :bh:
                        cleanup_dir = False

                        # We need a copy of lock_path because it is non-local
                        local_lock_path = lock_path
                        if not local_lock_path:
#                             local_lock_path = CONF.lock_path :bh:
                            local_lock_path = CONF_lock_path
...
#                             fileutils.ensure_tree(local_lock_path) :bh:
                            _ensure_tree(local_lock_path)
